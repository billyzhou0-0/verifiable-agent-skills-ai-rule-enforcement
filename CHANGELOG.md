...
- 规矩体检.py 诞生（15:59，0 token 本地脚本）：命名日期位置/引用同步/敏感信息/frontmatter/凭证转录五项检查，输出证据可粘贴汇报。/ 体检脚本诞生。

## [0.2.0] - 2026-08-10

### Pre-release (internal) / 开源前内部版本

- git pre-commit 红绿灯钩子：提交前自动体检、违规阻止提交（紧急 --no-verify 绕过）；钩子重建内容登记入册防丢失。/ 红绿灯落地。

## [0.3.0] - 2026-08-10

### Pre-release (internal) / 开源前内部版本

- SOUL.md 瘦身 25%（5,387→4,018 字）：细则搬入承接文件、注入层留指针+不可代码化核心；输出前体检表（自查区+盘点区）+ 声明附证据；AGENTS.md 镜像同步。/ 注入层设计落地。

## [1.0.0] - 2026-08-11

### Initial open-source release / 初始开源发布

- 开源发布：方法论 SKILL.md（三层架构）+ 通用化脚本（--vault 参数化）+ README（痛点故事+双语）+ MIT。/ 开源发布。
